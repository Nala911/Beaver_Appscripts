...
/**
 * Generates an AI Debugging Prompt for the most recent error.
 * Aggregates all logs tied to the last errored Run ID.
 */
function Logger_getLatestErrorDump() {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName(SHEET_NAMES.LOGS);
    if (!sheet || sheet.getLastRow() < 2) return { error: "No logs found." };

    var data = sheet.getDataRange().getValues();
    // Headers
    var headers = data[0];
    var runIdIdx = headers.indexOf('Run ID');
    var levelIdx = headers.indexOf('Level');
    
    var erroredRunId = null;
    
    // Find the latest error runId (start from bottom since we append rows now)
    for (var i = data.length - 1; i >= 1; i--) {
        if (data[i][levelIdx] === 'ERROR') {
            erroredRunId = data[i][runIdIdx];
            break;
        }
    }

    if (!erroredRunId || erroredRunId === 'N/A') {
        return { message: "No recent errors found to dump." };
    }

    // Collect all logs for this Run ID
    var runLogs = [];
    var mainErrorMsg = "";
    
    var startIdx = Math.max(1, data.length - 500);
    for (var i = startIdx; i < data.length; i++) {
        if (data[i][runIdIdx] === erroredRunId) {
            var row = data[i];
            runLogs.push(`[${row[headers.indexOf('Timestamp')]}] [${row[levelIdx]}] ${row[headers.indexOf('Source')]} (${row[headers.indexOf('Reference')]}): ${row[headers.indexOf('Message')]}`);
            if (row[levelIdx] === 'ERROR' && !mainErrorMsg) {
                 mainErrorMsg = row[headers.indexOf('Message')]; 
            }
        }
    }

    // Construct the markdown dump
    var dump = `Hello! Please help me fix this Google Apps Script error.\n\n`;
    dump += `## Error Synopsis\n\`\`\`text\n${mainErrorMsg}\n\`\`\`\n\n`;
    dump += `## Full Execution Log & Context (Run ID: ${erroredRunId})\n\`\`\`text\n`;
    dump += runLogs.join('\n'); // Show chronologically
    dump += `\n\`\`\`\n\n`;
    dump += `## Environment\n- User: ${Session.getActiveUser().getEmail() || 'Unknown'}\n- Script TimeZone: ${Session.getScriptTimeZone()}`;

    return { dump: dump, runId: erroredRunId };
}
