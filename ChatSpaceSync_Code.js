...
App.Engine.registerTool('CHAT_SYNC', {
    ...
    service: {
        getLoadData: function() {
            try {
                ...
                do {
                    ...
                    if (response.spaces) spacesList = spacesList.concat(response.spaces);
                    pageToken = response.nextPageToken;
                } while (pageToken);

                var uniqueSpaces = spacesList.map(function(s) {
                    return { id: s.name, name: s.displayName || s.name };
                });

                var prefs = App.Engine.getPrefs('CHAT_SYNC');
        ...
        savePreferences: function(spaceIds) {
            ...
            App.Engine.setPrefs('CHAT_SYNC', prefs);
            return _App_ok('Preferences saved.');
        },

        pullMembers: function(request) {
            var TARGET_SHEET_NAME = SHEET_NAMES.CHAT_SPACE_SYNC;
            var sheet = _App_ensureSheetExists('CHAT_SYNC');
            Logger.info('CHAT_SYNC', 'Pull Members', 'Pull started — spaces: [' + request.spaceIds.join(', ') + ']');
            ...
            request.spaceIds.forEach(function (spaceNameId) {
                try {
                    ...
                    do {
                        var response = _App_callWithBackoff(function() {
                            return Chat.Spaces.Members.list(spaceNameId, { pageToken: pageToken });
                        });
                        if (response.memberships) members = members.concat(response.memberships);
                        pageToken = response.nextPageToken;
                    } while (pageToken);
                    ...
                    members.forEach(function (m) {
                        ...
                        if (m.member && m.member.type === "HUMAN") { memberEmail = m.member.displayName || m.member.name; memberType = "User"; }
                        else if (m.groupMember) { memberEmail = m.groupMember.id; memberType = "Group"; }
                        else if (m.member && m.member.type === "BOT") { memberEmail = m.member.displayName || "Bot"; memberType = "Bot"; }
                    ...
                    Logger.info('CHAT_SYNC', 'Pull Members', 'Pulled ' + members.length + ' member(s) from space: ' + (space.displayName || space.name));
                } catch (err) {
                    Logger.error('CHAT_SYNC', 'Pull Members — ' + spaceNameId, err);
                }
            });

            SheetManager.overwriteObjects('CHAT_SYNC', outputObjects);
            this.savePreferences(request.spaceIds);
        ...
        pushChanges: function() {
            var stats = ExecutionService.processPendingRows('CHAT_SYNC', function(rowObj) {
                ...
                switch (action) {
                    case "ADD_MEMBER":
                        ...
                        var membership = { member: { name: "users/" + targetEmail, type: "HUMAN" }, role: targetRole };
            ...
            if (stats.processed === 0 && stats.errors === 0) return _App_ok("No data to sync.");
